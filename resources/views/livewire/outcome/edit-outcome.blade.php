<form class="md:w-1/2 space-y-5" wire:submit.prevent='editOutcome'>
    <div>
    ...
    <div>
        ...
        <select 
            ...
            @empty
            <option disabled selected>{{__("There are no income categories, you must create one")}}</option>
            @endforelse
        </select>
    ...
    <div>
        ...
        <div class="my-5 w-80">
            @if($image)    
                ...
                <img src="{{ asset('storage/outcomes/' . $image) }}" alt="{{ 'Outcome Image' . $outcome_description }}">  
            @endif        
        </div>
        <div class="my-5 w-80">
            @if ($new_image)
