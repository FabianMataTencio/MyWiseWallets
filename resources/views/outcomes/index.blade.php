<x-app-layout>
    ...
    class="flex flex-col gap-4 mt-16">
        ...
        <div class="flex gap-4 justify-center items-center mt-4">
            ...
            <div>
                <a href="{{ route('outcomes.create') }}" class="w-full py-2 px-4 bg-cyan-400 hover:bg-cyan-600 text-white rounded-full">
                    {{__("Create outcome")}}
                </a>
            </div>
            <div>
        ...
        <!-- Lista de Resultados -->
        <livewire:outcome.show-outcomes/>
        <p class="mt-3 text-gray-600 text-sm text-center text-wrap">{{ __('(Note: By default, only the outcome for the current month is displayed. If you want to see outcome from other months, you can use the filters.)') }}</p>
    </section>
</x-app-layout>
